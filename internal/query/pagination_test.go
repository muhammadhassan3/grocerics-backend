...
func TestPageFromContext(t *testing.T) {
	cases := []struct {
		...
		{"page=1", 1, DefaultPageSize},
		{"page=5&page_size=50", 5, 50},
		{"page=0", DefaultPage, DefaultPageSize},
		{"page=-3", DefaultPage, DefaultPageSize},
		{"page=abc", DefaultPage, DefaultPageSize},
		{"page_size=0", DefaultPage, DefaultPageSize},
		{"page_size=-1", DefaultPage, DefaultPageSize},
		{"page_size=abc", DefaultPage, DefaultPageSize},
		{"page_size=999", DefaultPage, MaxPageSize},
		{"page_size=100", DefaultPage, MaxPageSize},
		{"page_size=101", DefaultPage, MaxPageSize},
	}
	for _, c := range cases {
		t.Run(c.query, func(t *testing.T) {
			p := PageFromContext(newCtx(c.query))
...
func TestSortFromContext(t *testing.T) {
	...
	fallback := Sort{Column: "created_at", Direction: "desc"}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			s := SortFromContext(newCtx(c.query), allow, fallback)
...
func TestSortFromContext_EmptyAllowlist(t *testing.T) {
	fallback := Sort{Column: "created_at", Direction: "desc"}
	s := SortFromContext(newCtx("sort=name&order=asc"), nil, fallback)
...
func TestPage_OffsetLimit(t *testing.T) {
	cases := []struct {
		...
		{Page{Number: 1, Size: 20}, 0, 20},
		{Page{Number: 2, Size: 20}, 20, 20},
		{Page{Number: 5, Size: 7}, 28, 7},
	}
	for _, c := range cases {
...
func TestSort_OrderClause(t *testing.T) {
	got := Sort{Column: "name", Direction: "asc"}.OrderClause()
